...
function getSimpleFilter() {
  ...
  return { // see if some words in all words in original case match words in dict (good) or words in badList (bad), what's a badList? words a learner already learned?
...
//   find word-break on end of line which is seperated by '-', replace it with a normal word.
  s = s.replace(/([a-zA-Z]+)+-\n([a-zA-Z]+)/g, "$1$2\n");
  if (do_jump) { // user click parse and window scroll to demo area
    window.scroll(0, demo.parentNode.offsetTop - 30);
...
function elemExplain(
  ...
  if (cover) var explainHead = tailCover(voc, head, tail);
    ...
    info.audio.play();
    if (navigator.clipboard) navigator.clipboard.writeText(voc);
  }
...
function startRead() {
  ...
  readState.pop();
  readState.push(
    setTimeout(() => startRead(), info.audio.duration * (1 + rate) * 1000)
  );
}
